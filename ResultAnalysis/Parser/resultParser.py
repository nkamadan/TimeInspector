...
Filename = 'test_results_benign.txt'
...
def print_no_path_found_file_names():
    ...
    for line in line_data:
        if(len(line)>8 and line[0:6]=='File: ' and 'no path' in line):
            ...
            print(tempArray[0])
            #counter +=1
    ActualFileNameCount = set()    
    for line in line_data:
        if(len(line)>10 and line[0:9]=='FileName:'):
            ActualFileNameCount.add(line)
    print("\nActual FileName number ",len(ActualFileNameCount))
    print("\nActual No Path Len: ",len(NoPathSet))
    print('\n')
